...
function (Aloha, plugin, jquery, jquery19, UI, _) {
    ...
    var processTextNode = function ($elem, srch) {
    ...
    var walkNode = function ($container, srch) {
      $container.each(function (i, elem) {
        if (elem.nodeType ==3) {
          processTextNode(jquery(elem), srch);
        } else {
          walkNode(jquery(elem).contents(), srch);
        }
      });
    };

    var doSearch = function (what) {
      var $content = jquery("#contenteditor");

      walkNode($content.contents(), what);
    };

    var doReplace = function (replace) {
      var $content = jquery("#contenteditor");
    ...
    return plugin.create('findandreplace', {
        ...
        init: function () {
            ...
            $dialog.find('BUTTON.next-button').on('click', function () {
              var $content = jquery("#contenteditor");
              var search = $dialog.find('INPUT[name=find]').val().toLowerCase();
            ...
            $dialog.find('BUTTON.replace-button').on('click', function () {
              var $content = jquery("#contenteditor");
              var replace = $dialog.find('INPUT[name=replace]').val().toLowerCase();
