...
function (Aloha, plugin, jquery, jquery19, UI, _) {
    ...
    var doSearch = function (what, caseMatch) {
      ...
      findAndReplaceDOMText(document.getElementById('contenteditor'), {
        find: new RegExp(what, "g"+(caseMatch ? "" : "i")),
        replace: function(portion, match){
          ...
          if(portion.index !== 0){
            node.className += ' secondary-match';
          }
          return node;
        }
      });
    ...
    var doReplace = function (replace, selection) {
      ...
      selection.filter('.secondary-match').remove();
      selection.parent().next('.secondary-match').remove();
    ...
    var moveToMarker = function (num) {
      var $markers = jquery("#contenteditor").find('span.aloha-search-marker').not('.secondary-match');
        ...
        init: function () {
            ...
            $dialog.find('BUTTON.next-button').on('click', function () {
              ...
              if (cursorPosition == -1) {
                ...
                numberOfSearch = jquery("#contenteditor").find('span.aloha-search-marker').not('.secondary-match').length;
                cursorPosition = 0;
